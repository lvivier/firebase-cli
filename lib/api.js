...
Api.prototype.create = function (name, fn) {
  this.req('post', 'firebase/'+uri(name), fn)
}
...
Api.prototype.remove = function (name, fn) {
  this.req('delete', 'firebase/'+uri(name), {namespace: name}, fn)
}
...
Api.prototype.info = function (name, fn) {
  ...
  var fns = metrics.map(function (metric) {
    return function (cb) {
      if (error) return
      req('get', 'firebase/'+uri(name)+'/'+metric, query, function (err, res) {
...
Api.prototype.getToken = function (name, fn) {
  this.req('get', 'firebase/'+uri(name)+'/token', {namespace:name}, function (err, res) {
...
Api.prototype.getRules = function (name, fn) {
  this.getToken(name, function (err, token) {
    request
      .get('https://'+uri(name)+'.firebaseio.com/.settings/rules.json')
      .query({auth: token})
